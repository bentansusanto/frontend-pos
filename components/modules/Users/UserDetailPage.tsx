  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 px-4 py-8">
      ...
      <div className="bg-card text-card-foreground relative overflow-hidden rounded-2xl border shadow-sm">
        {/* Decorative background simple gradient */}
        <div className="from-primary/10 via-primary/5 absolute inset-x-0 top-0 h-32 bg-gradient-to-br to-transparent" />

        <div className="relative flex flex-col items-start gap-6 px-8 pt-12 pb-8 sm:flex-row sm:items-end">
          <div className="ring-background flex-shrink-0 rounded-full ring-4">
            <Avatar className="h-24 w-24 text-3xl shadow-md sm:h-32 sm:w-32 sm:text-4xl">
          ...
          <Card className="shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg">
                ...
                {isCashier ? "Cashier PIN Access" : "Login Credentials"}
              </CardTitle>
              <CardDescription>
                {isCashier
            ...
            <CardContent>
              {isCashier ? (
                <UpdatePinForm userId={userId} currentPin={user.pin} onSuccess={refetch} />
              ) : (
...
function UpdatePinForm({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        await updateUser({ id: userId, data: { pin: values.pin } }).unwrap();
    ...
    <form onSubmit={formik.handleSubmit} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <Input
            id="prof-address"
            placeholder="e.g. Jl. Contoh No. 1"
